...
- **middleware 인증 redirect**: `USE_MSW=true` 모드에선 보호 경로 마커 쿠키 검사 skip. 운영 빌드 (`USE_MSW=false`) 는 인증 마커 쿠키(`tripbite.authed`, auth-store 가 로그인 시 set / 로그아웃·만료 시 clear) 없으면 `/login?redirect=` 으로 SSR redirect. (Spring JSESSIONID 는 익명 요청에도 발급돼 "쿠키=로그인" 이 불성립 → middleware 는 FE 마커를 본다.) `src/middleware.ts` 의 `isMockMode` 분기 참조.
...
mock 의 `mockSignedIn` 모듈 state (`src/mocks/handlers.ts`) 가 `POST /auth/login` / `POST /auth/logout` 으로 토글되어 `/me`, `/mypage*`, `/letters*`, `/notifications` 등 보호 endpoint 가 미인증 분기. mock 환경에선 middleware 가 마커 쿠키 검사 skip 이므로, 보호 경로 진입 시 page mount 후 useMe / 다른 query 의 401 응답 → interceptor (mock skip) → MockAuthToggle 의 사용자 토글이 UX 진입점.

## 6. 트러블슈팅 — `/me 404` / 모든 API 404

Vercel 배포 직후 `/me` `/letters/...` 등 모든 API 가 404 라면 다음 순서로 확인:

### 6-1. env 가 실제 빌드에 inline 됐는지
...
- `MSW_ENABLED=true` 인데도 API 가 404 라면 6-2 / 6-4 항목으로.
