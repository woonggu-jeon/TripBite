...
**아직 검증하지 않은 영역** (다음 세션에서 대조 필요):

- `tokens/_typography.scss` ↔ Figma 텍스트 스타일 14개 (`Title/B_24_130%` 등). **Figma는 자간이 전부 음수(-2%, 12px 캡션만 -1%)** 인데 레포의 tracking 토큰과 맞는지 미확인
- `tokens/_misc.scss`의 `--radius-sm/md/lg/xl` ↔ Figma `Radius/{1,10,12,24,full}`. Figma는 **버튼·입력·카드 전부 12px 통일**
- `tokens/_layout.scss`의 spacing ↔ Figma `Spacing/{0..140}` (17단계, 4pt 그리드 아님)
