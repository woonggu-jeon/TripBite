...
| 흰 라벨 채움 버튼 면     | `--color-primary-strong`    | `#00B334` | 2.80:1 FAIL |
...
**2026-08-04 결정 — 채움 버튼은 시안값으로 되돌렸습니다.** 사용자 지시("피그마가 제일
우선", "버튼색 알아서 통일해줘")에 따라 `--color-primary-strong` 의 값을 `#00821F` →
`#00B334` 로 바꿨습니다. 즉 **흰 라벨 채움 버튼의 대비는 2.80:1 로 AA 미달**입니다
(버튼·칩·아이콘버튼·skip link 등 21개 사용처 공통).

토큰 이름은 그대로 두었으니, 대비를 되돌릴 때는 `tokens/_color.scss` 의
`--color-primary-strong` **한 줄만** `#00821F` 로 바꾸면 전부 복구됩니다.

초록 **텍스트**는 그대로 `--color-primary-text-bold`(5.08:1) 를 씁니다 — 지시는
버튼 색 통일이었고, 본문 텍스트까지 2.80:1 로 내리면 읽기 자체가 어려워집니다.
