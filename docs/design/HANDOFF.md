...
### 2단계 결론 — 결정이 더 필요합니다 (미착수)

토큰을 **추가**하는 것은 안전하지만, 위 드리프트를 실제로 해소하려면 앱 전역 시각 변경이
따릅니다. 그래서 2단계는 **대조까지만 하고 변경은 하지 않았습니다.**

| #   | 결정할 것                                                | 영향 규모        |
| --- | -------------------------------------------------------- | ---------------- |
| 5   | 행간 1.4 / 1.3 토큰을 추가하고 기존 1.35→1.4 로 옮기는가 | 17곳 + 전역 리듬 |
| 6   | `--radius-md` 를 8→12 로 바꾸는가, 70곳을 lg 로 옮기는가 | 70곳             |
| 7   | `--font-body-sm` 15→14, `--font-h3` 17→18 로 맞추는가    | 52곳             |
| 8   | Figma 의 그리드 밖 스페이싱(10/22/26/30…)을 도입하는가   | 토큰 규칙        |

**권장** — 5(행간 1.4 추가)와 10px 캡션 토큰 추가는 시안 재현에 반드시 필요하므로 진행.
6·7·8은 시안 충실도 대비 변경 규모가 커서 디자이너·PO 판단이 먼저입니다.
특히 6은 "8px 라운드"가 현재 앱의 인상을 만드는 요소라 단독 판단하면 안 됩니다.
