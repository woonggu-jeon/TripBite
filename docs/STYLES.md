...
| Primitive 컴포넌트     | `Card / Chip / IconButton / PageSection / Button / DestinationCard / ButtonGrid / TextField / PasswordField / MediaThumb / RadioGroup / Dialog / TabList+Tab+TabPanel` (`@/components/ui`) + `AuthLayout / AuthHeader` (`@/components/layout`) |
| 신규 토큰 (2026-06-22) | `--color-disabled` #E0E0E0 / `--color-text-disabled` #B4B4B4 / `--color-stroke` #C6C6C6 (Figma Stroke/01) / `--radius-input: 10px` (입력 폼 전용)                                                                                              |
