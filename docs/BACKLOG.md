...
- **2026-06-19**: 10-round 검증 Round 15-19. Round 15 (iOS PWA quirks): TextField input font-size 15→16px (iOS Safari auto-zoom 차단). Round 16 (animation): transform/opacity 기반 + prefers-reduced-motion 모두 정합 — fix 0. Round 17 (SEO deep): `/letter/[id]` HTML meta robots noindex 적용 (robots.ts disallow `/letter/` 와 이중 가드, 받은 편지 본문/location label 검색 노출 방지). Round 18 (DX): README/.env.example/scripts/orval/strict/hooks/extensions 모두 정합 — fix 0. Round 19 (사용자 시나리오): 13 finding 발견 — fix 큼 (편지 전송 후 새로고침 유실 BE id 보장 / 401 draft 유실 localStorage 정책 / 토너먼트 결과 저장 실패 재시도 / scroll restoration / 좋아요 cleanup) 모두 사용자 정책 결정 영역 또는 사이드이펙트 큰 작업 → BACKLOG.
- **2026-06-19**: iOS PWA splash 화면 추가. 사용자 요청. (1) Android 는 manifest 의 `background_color`/`theme_color`/192/512 icon 으로 OS 자동 splash 생성 — 추가 작업 0. (2) iOS Safari 는 manifest 자동 생성 X → `apple-touch-startup-image` link + 디바이스별 PNG 필요. (3) `scripts/generate-ios-splash.mjs` 신설 — sharp (^0.35.1 기설치, devDep 추가 0) 기반. `public/icons/icon-512x512.png` source 로 13 디바이스 사이즈 portrait PNG 일괄 생성 (iPhone SE/8Plus/X/XR/11ProMax/12mini/12-14/14ProMax/15ProMax + iPad/mini/Pro 11/Pro 12.9). 캔버스 short-edge 의 40% 비율 로고 center 합성, #ffffff 배경. 총 ~110KB. (4) `public/splash/*.png` 13장 추가. (5) `src/app/layout.tsx` 의 `generateMetadata().appleWebApp.startupImage` 에 디바이스별 media query 13건 추가 (device-width + device-height + pixel-ratio + portrait). Next.js Metadata API 가 `<link rel="apple-touch-startup-image">` 자동 render. (6) `package.json` scripts `gen:splash` 추가 — 디바이스 매트릭스 추가 시 재실행. 검증: tsc 0 / vitest 262 / build OK / prettier clean.
- **2026-06-19**: PasswordField primitive 신설 — 비밀번호 input 우측 visibility 토글 (Eye/EyeOff). 사용자 요청. (1) `TextField` 에 `trailingAdornment` slot 추가 — input 내부 우측 overlay 용 (suffix 와 별개, suffix 는 외부 row 의 중복확인 버튼). input padding-right 2.75rem 자동 확보 + .trailingAdornment absolute pointer-events 분리. (2) `PasswordField` 신설 (`@/components/ui`) — TextField wrap, 내부 `visible` state 로 type=password/text 토글, `Eye` / `EyeOff` (lucide-react) icon, aria-label i18n + aria-pressed, button type=button (submit 차단), 44x44 touch target. (3) 4 form 7 password input 일괄 적용 — LoginForm (1) / SignupForm (2: password + confirm) / ResetPasswordForm (2: FIELDS map) / ChangePasswordForm (3: current/new/confirm). LoginForm 의 `LoginField` type/`FIELDS` 배열은 username + password 타입 달라져 인라인 풀어씀 (TextField + PasswordField 분리). (4) i18n `common.showPassword` / `common.hidePassword` 추가 (ko/en, 626→628 키). (5) STYLES.md primitive 목록 갱신.
- **2026-06-19**: Bracket progress 라운드 단위 단순화 + 결승 layout shift fix. (1) 사용자 보고 (32강 시 31개 segment 부담) → `totalMatchesNeeded = N - 1` (전체 누적) → `roundMatchCount = round.matches.length` (라운드 한정). 32강=16, 16강=8, 8강=4, 4강=2 segment. 매 라운드 0% → 100% 채워지는 형태로 진행률 체감 향상. roundLabel("32강") + matchCount("1/16") 가 이미 라운드 + 매치 신호 동시 제공. (2) 결승(1 매치) progress 의미 없어 segment 숨김 처리 시 카드가 위로 14px 점프하는 layout shift 회귀 발견 → 빈 `<div className=progressBar aria-hidden />` placeholder 로 .progressBar height:6px + .head gap:0.5rem 유지 (background 없어 시각적 noise 0). 검증: tsc 0 / vitest 262 / prettier clean.
- **2026-06-19**: STYLES.md 정합 — `_chart.scss` 참조 / `--chart-1~8` 표 항목 (2곳) 제거. `--medal-{gold/silver/bronze}` Top5Card 전용 row 추가 (mode-invariant 글자색 정책 명시). [[docs-sync-after-code-change]] 정합.
- **2026-06-22**: 운영 SVG 첫 진입 지연 fix — `<Image>` (next/image) → native `<img>` 5 컴포넌트 (LoginForm logo / SignupCompleteView / FindIdForm / ForgotPasswordForm hero / LocationPermissionPrompt). 원인: next/image 의 `dangerouslyAllowSVG:false` (기본값) 가 SVG optimization 거부 + hydration 후 fetch 트리거 → 첫 진입 placeholder → 새로고침 후에야 cache hit. SVG 는 vector 라 optimization 불필요. native `<img>` 가 HTML 파싱 즉시 fetch + width/height 명시로 CLS 0. eslint-disable `@next/next/no-img-element` 주석으로 의도 명시.
- **2026-06-22**: BE 응답 (`BE_RESPONSE_auth_session_changes.md`) 정합. (1) interceptor 의 401 분기 status 단독 처리 → `data.code === 'AUTH_REQUIRED'` 만 자동 로그아웃. AUTH*INVALID_CREDENTIALS / 기타 401 은 silent reject — 호출처 폼 인라인 에러 (LoginForm 이미 구현). (2) `isAuthEndpoint()` helper — `/v1/auth/(login|signup|check-*|find-id|forgot-password|reset-password)`regex 매칭, 해당 endpoint 401 은 자동 로그아웃 skip (BE 응답 §4 루프 방지). (3) BE 가 401 응답에 Set-Cookie SID 자동 만료 — FE cookie 청소 호출 불필요. (4)`.gitignore` 에 BE\*REQUEST\**.md / BE*RESPONSE\*\_.md 패턴 추가 — [[be-request-response-no-commit]] 명문화.
- **2026-06-22**: favicon 갱신 — 디자이너 신규 `trip-bite-logo.svg` 기반. `scripts/generate-favicon.mjs` (sharp) → favicon-32 / apple-touch-icon (180) / icon-192/512 + root favicon.ico (32x32 PNG). 로고 캔버스 70% (좌우 15% 여백). layout.tsx `metadata.icons` 갱신 — SVG icon 우선 + PNG fallback 매트릭스. `npm run gen:favicon` 등록. Splash 도 어제 `d2b0eb2` 에서 동일 source 로 재생성된 13장 ✓ (실 iOS PWA 디바이스 검증 필요).
- **2026-06-22**: 401 자동 로그아웃 UX 강화. (1) interceptor 가 모든 경로의 401 에 `clearAuth()` 호출 — public 경로에서 stale store user 가 AppHeader 의 닉네임/아바타로 표시되던 거짓말 회귀 fix. (2) 세션 만료 toast — `auth:session-expired` window CustomEvent + `SessionExpiredWatcher` 컴포넌트 (providers 안 mount) 가 listen → `toast.warning(common.sessionExpired)` 한 세션 한 번만 (module-level flag 가 멀티 polling 폭주 차단). 보호 경로 hard nav 시 페이지 reload 로 flag 자연 reset. i18n `common.sessionExpired` (ko/en) 추가.
- **2026-06-22**: 회원가입 자동 로그인 분리 — 가입 ≠ 로그인 흐름 명시. (1) auth-store 에 `pendingSignupUser` + `setPendingSignupUser` 추가 (persist 미포함, 메모리 only → 새로고침 시 사라져 직접 진입 차단). (2) `useSignup.onSuccess` setAuth/setQueryData 제거 → setPendingSignupUser 만 + `router.replace('/signup/complete')`. (3) SignupCompleteView `useEffect` 가드 (pending 없으면 `/` redirect) + `handleStart` 가 명시적 setAuth + setQueryData + clear pending + `/onboarding`. (4) **middleware fix (실제 회귀 원인)**: `isAuthEntryPath` 가 `startsWith('/signup/')` 으로 `/signup/complete` 까지 auth-entry 분류 → SID 있는 사용자를 SSR 단계에서 `/` 로 redirect → 페이지 진입 자체 차단. `AUTH_ENTRY_PATHS.includes(pathname)` exact match 로 좁힘 (sub-path 통과). (5) AuthBootstrap 의 setAuth skip 분기 추가 (providers 에서 비활성 상태라 실제 dead code, 회귀 복원 시 유효하니 보존).
- **2026-06-22**: Figma 후속 fix 다수. (1) LoginForm 상단 logo 블록 누락 → Figma `#3355:184` 정합 (column gap 8 + 안 INSTANCE inner column gap 4: icon 40x37 위 / "여행한입" ExtraBold 20 아래 + tagline). `.card` wrapper 제거 (Figma 에 없는 외곽 border). (2) AuthHeader 신설 — Figma "FindIdA/FindPwA/FindId 결과" header (#3355:235): height 54, padding-left 10, 36x36 slot + 24x24 ChevronLeft, `router.back()` + history fallback. `Route` 타입 명시 (typedRoutes 정합). /find-id, /forgot-password page 적용 + AuthLayout variant="column" padding-top 0. (3) TextField `.label` Figma 정합 — text-sm medium fg → Bold 12 muted (#393939). (4) AuthForm `.form` gap 16→24, `margin: 0 auto` (column variant 좌측 정렬 회귀 fix). (5) Button `.s-lg` border-radius 8→12 (Figma button size 52 정합, sm/md 는 그대로). (6) OnboardingFlow dots progress 24x4 막대 → 8x8 round + 22x8 pill (active, width transition). (7) hero icon-title sp 22 / title-desc sp 12 — uniform gap 폐기 + 개별 margin. `.heroSignupComplete` modifier (sp 26 + title 23 — 회원가입 완료 변형). (8) TextField input height 46 + padding 0 14. (9) Figma 토큰 페이지 정합 — `--color-disabled` #DDDDDD→#E0E0E0, 신규 `--color-text-disabled #B4B4B4` / `--color-stroke #C6C6C6` (Stroke/01).
- **2026-06-19**: Figma "걸들의대모험's team library" (oG5wglySXPDrVmQIsQ9xYI) auth-flow 7 노드 일괄 적용. Phase A 토큰 정합 — `--color-border` `#C6C6C6`→`#D8D8D8` (디자이너 확정) + `--radius-input: 10px` 신규 (LOGIN 13px 미완 → 10 통일, button/card 영향 X). Phase B 아이콘 자산 5 SVG (trip-bite-logo / signup-complete-check / find-id-hero / find-pw-hero / location-hero) `public/images/auth/`. Phase C LoginForm — 3 link 가운데 정렬 + ellipsis dot (#c2c2c2) 구분, 좌측 회원가입 / 우측 분리 layout 폐기. Phase D FindIdForm + ForgotPasswordForm — 84px hero icon + ExtraBold 22 title + Regular 14 description (1.55 line, pre-line), `footLinkBack` primary SemiBold. i18n `heroTitle`/`heroDescription` 추가. Phase E LocationPermissionPrompt — lucide MapPin (28px) → 116px SVG hero, ExtraBold 24 title, 1.65 line desc, primary 위 + skip 아래 column button stack. Phase F 신규 화면 2종 — `/signup/complete` 라우트 + SignupCompleteView (96px check + 시작하기 CTA), useSignup onSuccess `/onboarding`→`/signup/complete`, FindIdForm 의 result view 를 Figma 카드 디자인 정합 (primary-tint bg + 라벨 + 26px ExtraBold username + client-side maskEmail). Phase G — Figma SPLASH 노드의 React intro 페이지 신설은 [[rendering-speed-first]] 위배로 도입 X. 대신 PWA OS splash 의 source 로고를 디자이너 신규 `trip-bite-logo.svg` 로 갱신, `npm run gen:splash` 재실행 (13 PNG). i18n 626→638 키. 8 commits (각 Phase 별 분리). 검증: tsc 0 / vitest 262 / build OK / prettier·eslint clean.
- **2026-06-19**: R11-R20 10라운드 audit (다른 시각). R11 SEO (30 page metadata + sitemap + robots dual-mode 정합) / R12 PWA (Serwist SW + offline fallback + manifest 정합, theme_color 분기 기존 BACKLOG) / R13 React Query — **fix 2건**: `use-onboarding.ts` + `use-mypage.ts` 의 raw `['auth','me']` queryKey 2건을 `authKeys.me()` factory 로 정합 (이전 audit 의 `mypageKeys.summary()` 동일 패턴 누락), test 도 같이 갱신 / R14 Zustand — **fix 1건**: `tournament-store` persist name `'tournament'` → `'tripbite.tournament'` (auth/ui/letter 와 prefix 정합, sessionStorage 라 무손실 transition) / R15 폼 (8 form zodResolver + 6 schema test 정합) / R16 라우팅 (middleware 5 책임 정합, `safeRedirectParam` helper 추출은 기존 BACKLOG) / R17 motion (50 사용처 중 41 가드, 차이 18 파일 모두 작은 transition — WCAG translate/zoom 위반 아님) / R18 fonts (Pretendard self-host + capsize fallback 정합) / R19 코드 품질 (TODO/FIXME/XXX **0**, console.\* 명시 prefix 분류) / R20 CI (6 workflows + size-limit + coverage 정합). 검증: tsc 0 / vitest 262 / prettier·eslint clean.
- **2026-06-19**: recharts 의존 + chart feature 일괄 제거. RegionWinsBarChart (recharts 백업 구현, 호출처 0) + `src/features/chart/` 폴더 전체 (BarChart/LineChart/PieChart + Impl + ChartSkeleton + colors + types) + `src/app/styles/_chart.scss` + `_dark.scss` 의 `--chart-2~8` + `globals.scss` 의 `_chart` import + `next.config.js` `optimizePackageImports` recharts + `package.json` `recharts ^2.13.0` + `i18n.chart.winsLabel` (다른 chart.\* 키는 RegionWinsChart 가 공유 — 보존). RegionWinsChart (CSS 가로 막대, recharts 미사용) 는 ranking 페이지에서 사용 중 — 보존. npm install: removed 31 packages. 검증: tsc 0 / vitest 262 / build OK / i18n 626 키 정합.
- **2026-06-19**: Top5Card / RecommendationBanner 부자연스러움 정리 + 10라운드 audit. (1) Top5Card hardcoded medal gradient (3종 hex) → 디자이너 신규 토큰 `--medal-gold/silver/bronze` + `color-mix(70%, fg)` 로 grad-end 자동 생성. (2) `.destination` font-size `--emoji-sm` (22px, emoji 토큰) → `--font-h3` (17px, semantic typography). (3) `.body` grid gap 0.25→0.375rem, `.cta` margin-top 제거 (grid gap 일원화), `.description` dead class 제거. (4) `RecommendationBanner.wrap` 양옆 들여쓰기 회귀 fix — `width: min(92%, 580px); margin: 0 auto` 제거 → `width: 100%` (부모 `.contentInner` 의 `--content-pad`/`--content-max` 가 이미 padding/cap 처리, 자식 92%는 이중 padding). (5) Top5Card medal dark mode contrast 회귀 fix — 글자색 토큰 (`--color-on-strong`/`--color-fg`) 가 dark 에서 1.1~2.6:1 로 떨어짐 → mode-invariant 진한 hex (#422006/#1f2937/#431407) 고정 (gold 7:1, silver 9:1, bronze 5:1). 디자이너 dark medal spec / `--medal-text-*` 토큰 도착 시 토큰화 BACKLOG. 10라운드 audit (R1 a11y / R2 perf / R3 보안 / R4 i18n / R5 mobile / R6 dark / R7 test / R8 docs / R9 error / R10 종합) — R6 에서 medal contrast 회귀 발견 후 즉시 fix, 나머지 9 라운드 신규 회귀 0.
- **2026-06-19**: 10-round 검증 Round 11-14. Round 11 (edge case 입력): `relative-time.ts` invalid ISO 시 `Number.isFinite()` 가드 + `formatKoreanDate` invalid date NaN 가드. Round 12 (async lifecycle): `use-service-worker-update` 의 updatefound + statechange listener 명시적 cleanup (ref-style 추적) + LetterComposeForm `void resolve().catch(()=>{})`. Round 13 (memory leak): 모든 timer/observer/blob URL/listener cleanup 정상 — fix 0. Round 14 (반응형 break point): 데스크톱 1024+ break point 부재 (모바일 우선 PWA 정합), `_mixins.scss` 의도적 자리잡이 보존, DestinationCard sizes 200px 정합 — fix 0.
- **2026-06-19**: 10-round 검증 Round 8 (dead code): knip 결과 62 unused files / 4 unused deps / 7 unused devDeps. 대부분 false positive — orval generated `*.msw.ts` (mock handler, 자동 생성) / Serwist `sw.ts` (next-pwa build) / `scripts/*.mjs` (npm script 직접 실행) / 의도적 보존 컴포넌트 (`AgeConfirmStep`, `ConsentBlock`, `PolicySection`, `ThemeSection`, `LanguageSwitcher`, `NicknameStep` 등 — 사용자 요청 복원 대비). 실 dead 식별 manual 검토 cost 큼 + 잘못 제거 시 회귀 risk → 보류. 진짜 unused dep 후보 (class-variance-authority/clsx/tailwind-merge/recharts) 도 chart/styling lib 로 실 사용 가능성 — 별도 검증 필요.
- **2026-06-19**: 10-round 검증 Round 3-7 진행. Round 3 (route guard): interceptor 401 hard redirect 시 현재 path 보존 (`/login?redirect=<encoded>`). Round 4 (prop API): Dialog className prop + IconButton loading prop (backward compat). Round 5 (toast): LetterComposeForm warning duration 2500 → toast type default (3000ms). Round 6 (analytics): `letter.sent` track 호출 + 폼 invalid 시 `haptic.warning()` (tap → warning). Round 7 (zustand SSR): finding 모두 보류 — cross-tab logout sync (storage event 리스너 추가 시 사용자 의도 결정 — 탭 A 로그아웃 시 탭 B 자동 reload), AuthGuard hydration race (이미 안전망 있음), UI theme FOUC (SSR cookie 패턴 필요 — sideeffect 큼). 추후 추적 영역 BACKLOG.
- **2026-06-19**: 10-round 검증 사이클 진행. Round 1 i18n deep: ForgotPasswordForm placeholder `you@example.com` 하드코딩 → `t('emailPlaceholder')` (ko/en 동기). 보류 — 동적 키 미검증 (category/season, generated enum 으로 type-safe), error-normalize i18n 화 (interceptor 구조 변경 큼), OG image locale 전달, SignupForm placeholder 스타일 통일 (사용자 톤 결정), IconButton aria-label i18n 확대 (컴포넌트별 검토). Round 2 CSS/SCSS 토큰: hardcoded spacing 12건 / box-shadow raw rgba 4건 / clamp 토큰화 가능 영역 — 모두 새 토큰 정의 필요 (디자인 결정 영역) + 시각회귀 baseline 흔들림 risk 로 보류. (Round 3+ 진행)
- **2026-06-19**: 5-dim audit 추가 진행 — TabList / RadioGroup 의 ARIA APG arrow key 패턴 + Error boundary cache reset. (a11y) `TabList` onKeyDown ArrowLeft/Right/Home/End 으로 focus + click (automatic activation 패턴), `RadioGroup` onKeyDown ArrowLeft/Right/Up/Down/Home/End. roving tabindex 미도입 (모든 button Tab 진입 유지) — 단순 추가 인터랙션. tablist/radiogroup wrapper 에 `tabIndex={-1}` (jsx-a11y eslint 충족, Tab 키 진입 안 함). (resilience) `src/app/error.tsx` 의 reset 호출 시 `queryClient.resetQueries()` 선행 — 같은 query cache fail 상태로 인한 동일 에러 재발 방지. 보류 (사이드이펙트 큼) — Carousel arrow key (viewport focusable + tab 순서 영향), SegmentError 의 cache reset (segment 별 영역 정책 결정).
- **2026-06-19**: 5-dimension audit (성능 deep / a11y deep / mobile UX / 데이터 fetching / 에러 resilience) 진행. 사이드이펙트 작은 fix 7건: (a11y) Carousel arrow/dots `:focus-visible` 추가, (mobile) AppHeader iconButton 40→44px, Banner close 36→44px, Dialog `max-height: calc(100svh - ...)` iOS 키보드 fallback, Carousel `overscroll-behavior: contain` (iOS bounce), (데이터) `useUpdateNickname` optimistic update + onError rollback, raw queryKey `['mypage','summary']` → `mypageKeys.summary()` (`use-ranking.ts` import), (resilience) `tournamentApi.recordResult` 가 axios signal 전달. 보류 (사이드이펙트 큼): Carousel/Tabs/RadioGroup arrow key 신설, Server RSC prefetch (i18n cookies → URL segment 재설계), Component-level Suspense, Mutation idempotency-key 확장 (BE 협의), Error boundary reset cache 정책, glass morphism contrast 검증, scroll/focus restoration. 검증: tsc 0 / vitest 262 / prettier·eslint clean.
- **2026-06-23**: BE 전달사항 정합 — POST `/v1/letters` (편지 작성) 에 `Idempotency-Key` 헤더 (client UUID) 추가. 토너먼트 `recordResult` (2026-06-19 합의) 와 동일 규약 — 24h 내 같은 키 = BE 가 동일 결과 반환 → 네트워크 재시도 / 더블 submit 시 letter 중복 생성 방지. generated `letterControllerComposeV1` 는 axios config override 불가 → `letterApi.send` 만 generated 우회 후 axios 직접 호출 (다른 endpoint 는 generated 그대로). `useSendLetter` mutationFn 안 `crypto.randomUUID()` 1회 생성 (mutate 1회 = 1 key, 더블 submit 은 LetterComposeForm isPending 가드로 1차 차단). 신규 test 1 case (헤더 UUID 형식 검증, vitest 19 → 20). MSW mock handler 는 토너먼트 mock 과 동일하게 키 검증 안 함 (BE 만 dedup).
- **2026-06-19**: BE 회신 (Idempotency-Key + 편지 ephemeral + 게스트 정리 안내) 정합 — POST `/v1/tournaments` 의 결과 기록에 `Idempotency-Key` 헤더 (client UUID) 추가. BE 가 24h 내 동일 키 = 동일 결과 반환 → 네트워크 재시도 / 더블탭에 랭킹 이중 카운트 방지. generated `tournamentControllerRecordV1` 는 axios config override 불가 (signal 만) → `tournamentApi.recordResult` 만 generated 우회 후 axios 직접 호출 (다른 endpoint 는 generated 그대로). `useRecordTournament` mutationFn 안 `crypto.randomUUID()` 1회 생성 (mutate 1회 = 1 key, 더블탭은 호출처 isPending 가드로 1차 차단). 신규 test 1 case (헤더 UUID 형식 검증). vitest 261 → 262. 편지 ephemeral 안내 (`letter.detail.autoDeleteNotice` "저장하지 않으면 3일 후 자동 삭제돼요") 는 이미 i18n + UI 적용 중 — 변경 없음. WeeklyRanking 제거 / 게스트 정리는 BE 내부 변경 — FE 영향 없음.
...
- **2026-06-17**: RegionHero heroImage 노출 + DestinationCard description/skeleton 보강:
  ...
  - **DestinationCardSkeleton 신설** — 카드와 동일한 `.card + .body` 구조 (image aspect-square + region + name 3줄 min + description 1줄). InfiniteList 에 `renderSkeleton` prop 추가, RegionDetailTabs 가 `renderSkeleton={DestinationCardSkeleton}` + `skeletonCount={4}` (2 columns × 2 rows). fetch ↔ 카드 전환 CLS 0.
  - **`.gitattributes` 신설 + Prettier endOfLine: 'lf' → 'auto'** — Windows core.autocrlf=true 환경에서 `npm run generate:api` 가 매번 generated 124 files 를 modified 로 표시하던 가짜 변경 해결. `* text=auto eol=lf` 로 line ending 일관화 + prettier 도 working tree EOL 그대로 받음.
  - **use-ranking.test.tsx +4 cases** — Br 0% 였던 useRanking / useTravelTypeQuiz / useWeeklyTopDestinations / useRecommendedDestinations cover. Coverage Br 69.89 → 70.43 (threshold 69 여유 0.89 → 1.43p 확대).
...
- **2026-06-14**: 14 영역 광범위 자율 진단 (read-only) + low ROI 1건 처리:
  ...
  - **진단 결과**: 14 영역 중 견고 11, 약점 3 (CSP enforce / letter key / dependency
    ...
    데이터 정합성 등 모두 견고. dependency major bumps (Next 16 / lucide 1 /
    hookform-resolvers 5) 는 RELEASE NOTES 사전 검토 필요한 별도 작업.
...
- **2026-06-14**: Cold start UX 보강:
  ...
  - 미적용 대상 명시 — `/quiz/result` 는 사용자별 결과라 cache 제외.
...
- **2026-06-25**: Figma sprite Icon 시스템 (45 icons, 34KB) — `public/icon-sources/{nav,bottom-modal,detail,circle,header,ui}` 폴더 통합 + `build-icons.mjs` LOCAL_OVERRIDES + COLOR_PATTERN 자동 `currentColor` 변환. lucide-react → `<Icon name="..." />` 일괄 교체 (production + stories). 토너먼트 PNG migration (intro circle-stack 4계절 색 + SeasonIcon + FallingPetals + ChungbukMap drop). NotificationsClient row 재구성 (notiCircle 44 + relative time). EmptyState hero variant 통일 (size 36). 캐러셀 bleed-right (HomeRec/RelatedDest).
- **2026-06-25 자율 검토 8라운드** — multi-dimension sub-agent (a11y/dark/responsive/regression/perf/CSS/test/store/SVG/i18n/memo/type/CLS/event-leak/focus-trap/Suspense). 25+ 회귀 fix: dark mode hardcode 색 token 화 / 청주 4-path 매핑 hijack 차단 / SVG fetch fallback + CLS 회피 / focus-visible / `[data-season]` scope / store selector 통합 / type guard / visited useMemo / React.memo (DestinationCard/LetterRowCard/WinnerCard) / aria-label i18n / ChungbukMap/StampMap keyboard 접근 (tabIndex + Enter handler) / Top1Hero priority. 종합 검증 — test 253/253 + build pass.
