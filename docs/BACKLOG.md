...
| 12   | **`/destinations/:id` (detail) spec 합의** | 기본 (`id / name / category / region / imageUrl(대표사진) / description`) + 부가 (`photos[](추가갤러리) / address / openingHours / closedDays / admissionFee / parkingAvailable / phone / website / coords{lat,lng} / tags[] / rating{value,count} / bestSeasons[]`). 부가 모두 optional.                       | field 명 **long form** 통일. **`summary` 폐기 → `description` 사용**. **대표사진은 `imageUrl`**, `photos[]` 는 갤러리 (없으면 imageUrl 단일 hero). `src/features/tournament/schemas/destination.ts` 가 진실.               |
