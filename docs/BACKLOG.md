...
- **2026-06-05**: BE 실 구현 (`docs/API_CONTRACT.md`) 와 FE 정합 — sessionID 단일 쿠키 / detail spec (`summary`/`restDate`/`parking`) / User 응답 (`username`/`avatarUrl`/`travelType`) / seed id `tour-<contentid>` / `DELETE /me` / weather 폐기
...
| 14   | **generated stale endpoint**                  | `src/api/generated/auth/auth.{ts,msw.ts}` 에 폐기된 `/auth/refresh` 잔존. `DestinationControllerDetailV1200 = {[key: string]: unknown}` (BE swagger 미정의).                                                                                                                                                                                                                                                                                                                                                           | BE swagger 정합 후 `npm run generate:api` 재실행 — orval 자동 동기화. 수동 삭제는 무의미 (재생성).                                                                                                                                                                                                                                                                              |
| 17   | **orval 마이그 선행 — BE swagger 정합 요청**  | `npm run generate:api` 실행 후 검토 결과: ① **빈 response schema 5종** — `DestinationControllerDetailV1200` / `LocationControllerReverseV1200` / `QuizControllerGetMeV1200` / `RankingControllerListV1200` / `RecordTournamentDtoTheme`. ② **거의 모든 endpoint 응답 `void`** — `meControllerGetMeV1` 등이 User 못 받음. ③ **multipart spec 누락** — `meControllerUploadAvatarV1` 가 file 인자 없이 generated. ④ **폐기된 endpoint 잔존** — `auth/refresh` / `weather/current` / `location/ip` swagger 에서 제거 필요. | BE 가 NestJS 컨트롤러에 `@ApiResponse({ type: XxxDto })` / `@ApiBody({ type: XxxDto })` + multipart `@ApiConsumes('multipart/form-data')` + file DTO 명시 추가. swagger 갱신 후 FE 가 `npm run generate:api` 재실행 → feature 별 점진 마이그 (auth → mypage → notification → letter → region → ranking → tournament 순). 현재 상태로 마이그하면 type 안전성 0 + multipart 깨짐. |
| 15   | **README.md 대규모 정합**                     | sessionID 모델 전환 후 본문에 잔존한 JWT/refresh/weather 참조 (15+ 곳). 헤더만 정합 완료. 본문 endpoint 표 / Auth 섹션 / weather feature 명시 갱신 필요.                                                                                                                                                                                                                                                                                                                                                               | BE 통합 검증 완료 후 일괄 갱신. 작업 분량 **M** (1-3시간).                                                                                                                                                                                                                                                                                                                      |
